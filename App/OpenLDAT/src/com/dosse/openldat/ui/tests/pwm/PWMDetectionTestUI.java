 ...
 */
package com.dosse.openldat.ui.tests.pwm;
...
import com.dosse.openldat.tests.pwm.PWMDetectionTest;
 ...
 * @author dosse
 */
public abstract class PWMDetectionTestUI extends javax.swing.JFrame {

    private String asText = "";

    /**
     * Creates new form PWMDetectionTestUI
     */
    public PWMDetectionTestUI(double freq, boolean noise, int[] raw, double sampleRate) {
        initComponents();
        String s = "<html>";
        if (freq <= 0) {
            s += "No PWM detected";
            if (noise) {
                s += ", but noise was detected";
            }
        } else {
            s += "PWM Frequency: " + String.format("%.2f", freq) + " Hz";
        }
        s += "</html>";
        jLabel2.setText(s);
        jLabel1.setVisible(freq > 0 || noise);
        chart1.addChannel(new Channel(new ArrayBuffer(raw), 0, 1023, new Color(255, 255, 255)));
        chart1.setGrids(true, 0, ((double) raw.length / sampleRate), ((double) raw.length / sampleRate) * 0.1, 1, true, 0, 1023, 128, 0);
        chart1.setUnits("s", "");
        StringBuilder b = new StringBuilder();
        for (int i = 0; i < raw.length; i++) {
            b.append((((double) i) / sampleRate) + "\t" + raw[i] + "\r\n");
        }
        asText = b.toString();
    ...
    private void initComponents() {
        ...
        setTitle("PWM Detection Test - Results");
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder("Raw data"));
        ...
        jLabel1.setText("<html>Note: PWM and noise will affect the accuracy of most tests.</html>");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        c.setSelectedFile(new File("PWM - " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HHmmss")) + ".txt"));
        ...
        try {
            ...
            w.write(jLabel2.getText().replace("<html>", "").replace("</html>", "") + "\r\n\r\n");
            w.write("Time\tBrightness\r\n");
            w.write(asText);
            w.flush();
            w.close();
    ...
    public static void run(Device d, Runnable doneCallback) {
        ...
        PWMDetectionTest test = new PWMDetectionTest(d) {
            @Override
            public void onDone(Map results) {
                EventQueue.invokeLater(new Runnable() {
                    @Override
                    public void run() {
                        PWMDetectionTestUI ui = new PWMDetectionTestUI((double) results.get("frequency"), (boolean) results.get("noisy"), (int[]) results.get("raw"), (double) results.get("sampleRate")) {
                            @Override
                            public void onClose() {
                                doneCallback.run();
                            }
