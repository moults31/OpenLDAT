 ...
 */
package com.dosse.openldat.ui.tests.pixelresponse;
 ...
 * @author dosse
 */
public abstract class PixelResponseTestResults extends javax.swing.JFrame {

    private JTable jTable1;
    private String asText;

    /**
     * Creates new form PixelResponseTestResults
     */
    public PixelResponseTestResults(int[] steps, double[][] ms, String method, boolean flickeringDetected) {
        initComponents();
        ...
        asText += "\r\n";
        Object[][] rows = new Object[steps.length][steps.length + 1];
        for (int row = 0; row < rows.length; row++) {
            rows[row][0] = "To " + steps[row];
            asText += steps[row] + "\t";
            for (int col = 1; col < rows[0].length; col++) {
                try {
                    rows[row][col] = String.format("%.2f ms", ms[row][col - 1]);
                    asText += String.format("%.2f", ms[row][col - 1]) + "\t";
                } catch (Throwable t) {
                    rows[row][col] = "Error";
                    asText += "Error\t";
                }
    ...
    private class ColoredCellRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
            try {
                String ms = (String) value;
                double d = Double.parseDouble(ms.substring(0, ms.length() - 2));
                if (Double.isFinite(d)) {
                    d = (d < 0 ? 0 : d > 30 ? 30 : d) / 30.0;
    ...
    private void initComponents() {
        ...
        setTitle("Pixel Response Time Test - Results");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        c.setSelectedFile(new File("Pixel Response Times - " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HHmmss")) + ".txt"));
